...
(node ../server.js --port=43317 > tmpfiles/server_output) & sleep 1

# Writing subtests

# TEST: Run curl on app/flip
echo "Running session for ${BLUE}app/flip${NC}\n"
result=$(curl http://localhost:43317/app/flip)
...
result=$(curl -I http://localhost:43317/app/flip)
...
result=$(curl -I http://localhost:43317/app/invalid)
...
result=$(curl http://localhost:43317/app/invalid)
expected=$(echo "404 [Nn][Oo].*[Nn][Dd]")
match="$(echo $result | grep -E $expected)"
...
# TEST: Random flips
echo "Running session for ${BLUE}app/flips/117 (generated randomly)${NC}\n"
result=$(curl http://localhost:43317/app/flips/117)
expected=$(echo "{['\"]?raw['\"]?:\s*\[((['\"]?tails['\"]?|['\"]?heads['\"]?),?){117}\],\s*['\"]?summary['\"]?:{(['\"]?tails['\"]?:\d{1,6},?|['\"]heads['\"]:\d{1,6},?){1,2}}}")
match="$(echo $result | grep -E $expected)"
[ -n "$match" ] && ( echo "${GREEN}Passed app/flips/117 (generated randomly)${NC}" && echo "${GREEN}app/flips/117 (generated randomly)${NC}" >> tmpfiles/passed ) || ( echo "${RED}Failed app/flips/117 (generated randomly)${NC}" &&  echo "${RED}app/flips/117 (generated randomly)${NC}" >> tmpfiles/failed &&
  echo "Expected: $expected" >> tmpfiles/failed && echo "Result: $result" >> tmpfiles/failed ) 

# TEST: Call Heads
echo "Running session for ${BLUE}app/flip/call/heads${NC}\n"
result=$(curl http://localhost:43317/app/flip/call/heads)
...
result=$(curl http://localhost:43317/app/flip/call/tails)
expected=$(echo "(win|lose)")
match="$(echo $result | grep -E $expected)"
...
result=$(curl http://localhost:43317/app/flip/call/invalid)
expected=$(echo "404 [Nn][Oo].*[Nn][Dd]")
match="$(echo $result | grep -E $expected)"
...
result=$(curl http://localhost:43317/app/log/access)
expected=$(echo "404 [Nn][Oo].*[Nn][Dd]")
match="$(echo $result | grep -E $expected)"
...
result=$(curl http://localhost:43317/app/error)
expected=$(echo "404 [Nn][Oo].*[Nn][Dd]")
match="$(echo $result | grep -E $expected)"
...
# Finished subtests
ps | grep "node ../server.js --port=43317" | grep -v grep | awk '{print $1}' | read pid
; kill $pid
result=$(cat tmpfiles/server_output)
expected=$(echo "App listening on port 43317")
match="$(echo $result | grep -E $expected)"
...
(node ../server.js --debug=true --port=13312 > tmpfiles/server_output) & sleep 1

# Writing subtests

# TEST: Log access
echo "Running session for ${BLUE}/app/log/access on debug session${NC}\n"
result=$(curl http://localhost:13312/app/log/access)
...
result=$(curl http://localhost:13312/app/error)
...
# Finished subtests
ps | grep "node ../server.js --debug=true --port=13312" | grep -v grep | awk '{print $1}' | read pid
; kill $pid
result=$(cat tmpfiles/server_output)
expected=$(echo "App listening on port 13312")
match="$(echo $result | grep -E $expected)"
