...
(node ../server.js --port=28892 > tmpfiles/server_output) & sleep 1

# Writing subtests

# TEST: Run curl on app/flip
echo "Running session for ${BLUE}app/flip${NC}\n"
result=$(curl http://localhost:28892/app/flip)
...
result=$(curl -I http://localhost:28892/app/flip)
...
result=$(curl -I http://localhost:28892/app/invalid)
...
result=$(curl http://localhost:28892/app/invalid)
expected=$(echo "404 [Nn][Oo].*[Nn][Dd]")
match="$(echo $result | grep -E $expected)"
...
# TEST: Random flips
echo "Running session for ${BLUE}app/flips/146 (generated randomly)${NC}\n"
result=$(curl http://localhost:28892/app/flips/146)
expected=$(echo "{['\"]?raw['\"]?:\s*\[((['\"]?tails['\"]?|['\"]?heads['\"]?),?){146}\],\s*['\"]?summary['\"]?:{(['\"]?tails['\"]?:\d{1,6},?|['\"]heads['\"]:\d{1,6},?){1,2}}}")
match="$(echo $result | grep -E $expected)"
[ -n "$match" ] && ( echo "${GREEN}Passed app/flips/146 (generated randomly)${NC}" && echo "${GREEN}app/flips/146 (generated randomly)${NC}" >> tmpfiles/passed ) || ( echo "${RED}Failed app/flips/146 (generated randomly)${NC}" &&  echo "${RED}app/flips/146 (generated randomly)${NC}" >> tmpfiles/failed &&
  echo "Expected: $expected" >> tmpfiles/failed && echo "Result: $result" >> tmpfiles/failed ) 

# TEST: Call Heads
echo "Running session for ${BLUE}app/flip/call/heads${NC}\n"
result=$(curl http://localhost:28892/app/flip/call/heads)
...
result=$(curl http://localhost:28892/app/flip/call/tails)
expected=$(echo "(win|lose)")
match="$(echo $result | grep -E $expected)"
...
result=$(curl http://localhost:28892/app/flip/call/invalid)
expected=$(echo "404 [Nn][Oo].*[Nn][Dd]")
match="$(echo $result | grep -E $expected)"
...
result=$(curl http://localhost:28892/app/log/access)
expected=$(echo "404 [Nn][Oo].*[Nn][Dd]")
match="$(echo $result | grep -E $expected)"
...
result=$(curl http://localhost:28892/app/error)
expected=$(echo "404 [Nn][Oo].*[Nn][Dd]")
match="$(echo $result | grep -E $expected)"
...
# Finished subtests
ps | grep "node ../server.js --port=28892" | grep -v grep | awk '{print $1}' | read pid
; kill $pid
result=$(cat tmpfiles/server_output)
expected=$(echo "App listening on port 28892")
match="$(echo $result | grep -E $expected)"
...
(node ../server.js --debug=true --port=16912 > tmpfiles/server_output) & sleep 1

# Writing subtests

# TEST: Log access
echo "Running session for ${BLUE}/app/log/access on debug session${NC}\n"
result=$(curl http://localhost:16912/app/log/access)
...
result=$(curl http://localhost:16912/app/error)
...
# Finished subtests
ps | grep "node ../server.js --debug=true --port=16912" | grep -v grep | awk '{print $1}' | read pid
; kill $pid
result=$(cat tmpfiles/server_output)
expected=$(echo "App listening on port 16912")
match="$(echo $result | grep -E $expected)"
...
(( cp ../access.log ./ ) & ( node ../server.js --log=true --port=35328 > tmpfiles/server_output )) & sleep 1

# Writing subtests

# TEST: Log access
echo "Running session for ${BLUE}/app/log/access on non debug session${NC}\n"
result=$(curl http://localhost:35328/app/log/access)
...
# Finished subtests
ps | grep "node ../server.js --log=true --port=35328" | grep -v grep | awk '{print $1}' | read pid
; kill $pid
...
(( cp ../access.log ./ ) & ( node ../server.js --log=false --port=30514 > tmpfiles/server_output )) & sleep 1

# Writing subtests

# TEST: Log access
echo "Running session for ${BLUE}/app/log/access on non debug session${NC}\n"
result=$(curl http://localhost:30514/app/log/access)
expected=$(echo "404 NOT FOUND")
match="$(echo $result | grep -E $expected)"
[ -n "$match" ] && ( echo "${GREEN}Passed /app/log/access on non debug session${NC}" && echo "${GREEN}/app/log/access on non debug session${NC}" >> tmpfiles/passed ) || ( echo "${RED}Failed /app/log/access on non debug session${NC}" &&  echo "${RED}/app/log/access on non debug session${NC}" >> tmpfiles/failed &&
  echo "Expected: $expected" >> tmpfiles/failed && echo "Result: $result" >> tmpfiles/failed ) 
...
# Finished subtests
ps | grep "node ../server.js --log=false --port=30514" | grep -v grep | awk '{print $1}' | read pid
; kill $pid
